...
public class FoldTextView extends AppCompatTextView {
    ...
    public FoldTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (TextUtils.isEmpty(mExpandText)) {
            mExpandText = getContext().getString(R.string.part_text);
        }
        if (TextUtils.isEmpty(mFoldText)) {
            mFoldText = getContext().getString(R.string.full_text);
        }
        if (mTipGravity == END) {
    ...
    private void translateText(Layout layout, BufferType type) {
        ...
        if (layout.getLineCount() > mShowMaxLine) {
            ...
            if (mTipGravity == END) {
                ...
                //防止超长崩溃
                while (end <= mOriginalText.length() && layout.getPrimaryHorizontal(end - 1) + getTextWidth(mOriginalText.subSequence(end - 1, end).toString()) < x) {
                    end++;
                }
                end--;
            } else {
