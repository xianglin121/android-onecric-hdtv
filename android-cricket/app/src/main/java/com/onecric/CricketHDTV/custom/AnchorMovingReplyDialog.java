...
import com.onecric.CricketHDTV.activity.LiveMovingCommentActivity;
 ...
 * 时间：2021/11/18
 */
public class AnchorMovingReplyDialog extends Dialog implements View.OnClickListener {
    ...
    public AnchorMovingReplyDialog(@NonNull Context context, int themeResId) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.ll_reply_input:
                if (scanForActivity(getContext()) instanceof LiveMovingCommentActivity) {
                    ((LiveMovingCommentActivity)scanForActivity(getContext())).showInputDialog(InputCommentMsgDialogFragment.TYPE_REPLY, mMovingBean.getId());
                }
                break;
            case R.id.ll_like:
                if (mMovingBean != null) {
                    ...
                    if (scanForActivity(getContext()) instanceof LiveMovingCommentActivity) {
                        ((LiveMovingCommentActivity)scanForActivity(getContext())).doLike(mMovingBean.getId());
                    }
                }
                break;
        }
    ...
    private void getList(int page, int id) {
        ...
        ApiClient.retrofit().create(ApiStores.class)
                .getMovingReplyList(CommonAppConfig.getInstance().getToken(), getRequestBody(jsonObject))
    ...
    public void doLike(int id) {
        ...
        jsonObject.put("id", id);
        ApiClient.retrofit().create(ApiStores.class)
                .doMovingCommentLike(CommonAppConfig.getInstance().getToken(), getRequestBody(jsonObject))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(new ApiCallback() {
                    @Override
