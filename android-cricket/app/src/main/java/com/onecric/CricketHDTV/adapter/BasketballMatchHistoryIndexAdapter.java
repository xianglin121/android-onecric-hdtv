...
import com.onecric.CricketHDTV.model.HistoryIndexInnerBean;
 ...
 * 时间：2021/9/11
 */
public class BasketballMatchHistoryIndexAdapter extends BaseMultiItemQuickAdapter<HistoryIndexInnerBean, BaseViewHolder> {

    public BasketballMatchHistoryIndexAdapter(List<HistoryIndexInnerBean> data, int type) {
        super(data);
        if (type == BasketballMatchIndexInnerFragment.TYPE_DAXIAOQIU) {
    ...
    @Override
    protected void convert(@NonNull BaseViewHolder helper, HistoryIndexInnerBean item) {
        switch (helper.getItemViewType()) {
            case MatchListBean.HEAD:
                helper.addOnClickListener(R.id.cl_company);
                if (!TextUtils.isEmpty(item.getWin_rate())) {
                    helper.setText(R.id.tv_win_rate, item.getWin_rate());
                }else {
                    helper.setText(R.id.tv_win_rate, "");
                }
                if (!TextUtils.isEmpty(item.getLoss_rate())) {
                    helper.setText(R.id.tv_lose_rate, item.getLoss_rate());
                }else {
            ...
            case MatchListBean.CONTENT:
                ...
                tv_home_rate.setText(String.valueOf(item.getHome()));
                tv_flat_rate.setText(String.valueOf(item.getFlat()));
                tv_away_rate.setText(String.valueOf(item.getAway()));
                ...
                if (!TextUtils.isEmpty(item.getHome_percentage())) {
                    tv_home_percent.setText(item.getHome_percentage() + "%");
                }else {
                    tv_home_percent.setText("");
                }
                if (!TextUtils.isEmpty(item.getFlat_percentage())) {
                    tv_flat_percent.setText(item.getFlat_percentage() + "%");
                }else {
                    tv_flat_percent.setText("");
                }
                if (!TextUtils.isEmpty(item.getAway_percentage())) {
                    tv_away_percent.setText(item.getAway_percentage() + "%");
                }else {
