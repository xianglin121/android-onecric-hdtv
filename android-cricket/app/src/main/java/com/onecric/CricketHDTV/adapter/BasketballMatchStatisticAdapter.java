...
public class BasketballMatchStatisticAdapter extends BaseQuickAdapter<StatisticBean, BaseViewHolder> {
    public BasketballMatchStatisticAdapter(int layoutResId, @Nullable List<StatisticBean> data) {
        ...
        score_pb.postDelayed(new Runnable() {
            ...
            public void run() {
                ...
                if (Integer.valueOf(item.getHomeData()) > Integer.valueOf(item.getAwayData())) {
                    leftColor = mContext.getResources().getColor(R.color.c_687AE0);
                    rightColor = mContext.getResources().getColor(R.color.c_4DF84A4B);
                }else if (Integer.valueOf(item.getHomeData()) < Integer.valueOf(item.getAwayData())) {
                    leftColor = mContext.getResources().getColor(R.color.c_4D687AE0);
                    rightColor = mContext.getResources().getColor(R.color.c_F84A4B);
                }else {
                    leftColor = mContext.getResources().getColor(R.color.c_687AE0);
                    rightColor = mContext.getResources().getColor(R.color.c_F84A4B);
                }
