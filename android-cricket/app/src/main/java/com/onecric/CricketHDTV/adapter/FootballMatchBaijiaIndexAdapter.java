...
import com.onecric.CricketHDTV.fragment.FootballMatchIndexInnerFragment;
 ...
 * 时间：2021/9/11
 */
public class FootballMatchBaijiaIndexAdapter extends BaseMultiItemQuickAdapter<BasketballIndexOneBean, BaseViewHolder> {

    private int mType;
    private int mId;

    public FootballMatchBaijiaIndexAdapter(List<BasketballIndexOneBean> data, int type, int id) {
        super(data);
        mType = type;
        mId = id;
        if (type == FootballMatchIndexInnerFragment.TYPE_RANGQIU) {
            addItemType(MatchListBean.HEAD, R.layout.item_football_baijia_rangqiu_index_head);
        }else if (type == FootballMatchIndexInnerFragment.TYPE_OUPEI) {
            addItemType(MatchListBean.HEAD, R.layout.item_football_baijia_oupei_index_head);
        }else if (type == FootballMatchIndexInnerFragment.TYPE_DAXIAOQIU) {
                ...
                if (mContext.getString(R.string.highest_value).equals(item.getCompany_name())
                    ...
                    helper.getView(R.id.cl_company).setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View v) {
                            RangqiuIndexDetailActivity.forward(mContext, RangqiuIndexDetailActivity.TYPE_FOOTBALL, item.getCompany_id(), mId, mType);
                        }
                    });
                    if (position%2 == 0) {
